...
@Component({
  selector: 'app-home',
  templateUrl: './viewleaveapp.component.html',
})
export class LeaveApplistComponent  {
  ...
  ngOnInit() {
    ...
    this.isLoading = true;
    this.commonservice.getallleavapp(this.postsPerPage, this.currentPage);
    this.commonservice
  ...
  openDialog(element) {
    this.model=element;
      ...
      this.dialog.open(EditLeavappContent,dialogConfig
    );
...
@Component({
  ...
  templateUrl: 'editleaveapp_dialog.html',
})

export class EditLeavappContent{ 
